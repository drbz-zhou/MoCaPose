...
public:
    ...
    unsigned long Read(uint8_t Ch, uint8_t Add);

private:
    void Channel_Select(uint8_t Ch_Mask, uint16_t Sample_Rate, uint8_t Add);
    void write_Cap(uint16_t address, uint16_t data, uint8_t Add);
    uint16_t read_Cap(uint16_t address, uint8_t Add);
